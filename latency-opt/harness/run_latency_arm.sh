...
run_one() {  # $1 bench, $2 task_id
  ...
  # ---- arm-specific env ------------------------------------------------------
  local sock="/tmp/czhai_shelld/$ARM.$bench.$tid/sock"
  (
    if [[ $ARM == B || $ARM == C ]]; then
    ...
    # no `set -e` in this script: an unguarded failed cd would run codex in
    # the caller's cwd and silently corrupt the arm's numbers
    cd "$workdir" || { echo "FATAL: cd $workdir failed" >&2; echo 97 > "$run_dir/exit_code"; exit 97; }
  ...
  # ---- teardown --------------------------------------------------------------
  # kill children first: a speculated command still in flight would otherwise
  # outlive its worker (same pattern as run_option_b.sh)
  local pf pid
  for pf in spec_early.pid spec.pid respec.pid; do
    [[ -f "$run_dir/$pf" ]] || continue
    pid=$(cat "$run_dir/$pf")
  ...
  local wall
  wall=$(grep -m1 "Elapsed" "$run_dir/time.txt" 2>/dev/null | awk '{print $NF}')
